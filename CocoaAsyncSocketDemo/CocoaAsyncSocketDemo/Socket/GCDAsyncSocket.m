...
// 判断 GCDAsyncSocketLoggingEnabled 宏标记是否可用
...
// 结束 #if GCDAsyncSocketLoggingEnabled 宏判断
#endif
...
// Manually Evaluate Trust : 手动评估信任
...
{
	kSocketStarted                 = 1 <<  0,  // If set, socket has been started (accepting/connecting) (接受/开始连接)
	kConnected                     = 1 <<  1,  // If set, the socket is connected (已经连接)
...
{
	kIPv4Disabled              = 1 << 0,  // If set, IPv4 is disabled （IPv4 禁用）
	kIPv6Disabled              = 1 << 1,  // If set, IPv6 is disabled   （IPv4 禁用）
	kPreferIPv6                = 1 << 2,  // If set, IPv6 is preferred over IPv4  (首选 IPv6)
	kAllowHalfDuplexConnection = 1 << 3,  // If set, the socket will stay open even if the read stream closes(如果设置,套接字保持开放,即使读流关闭)
};
                ...
                error:(NSError **)errPtr
{
	...
	// Just in case immutable objects were passed  ( 以防不可变对象被传递 )
	NSString *host = [inHost copy];                 // 主机
	NSString *interface = [inInterface copy];       // 接口
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		#pragma clang diagnostic(诊断) pop
		}});
